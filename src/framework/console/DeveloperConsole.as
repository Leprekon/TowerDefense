...
package framework.console
{
	...
	public class DeveloperConsole extends Sprite
	{
		...
		private const colour_type:String = "0ffd906";
		private const colour_param_type:String = "B5B5B5";

		private const VERSION_NAME:String = "Developer Console"
		private const HELP:String = " - Type 'clear' to clear the console\n" +
									...
									" - Type \"list\" to get the base commands list" +
		...
		// Constructor
		public function DeveloperConsole(_main:*, parent:Sprite)
		{
			main = _main;
			_parent = parent;	// Start off point for seeing/using variables and functions (main class or frame/stage)
			...
			historyText.width = StageUtil.stage.stageWidth;
			...
			inputText.width = StageUtil.stage.stageWidth;
			inputText.height = 21;
			...
			suggestText.multiline = true;
				...
				// Tracer
			tracerAlignX = StageUtil.stage.stageWidth - 5;
			tracerAlignY = 5;
			...
			StageUtil.stage.addEventListener(Event.RESIZE, handleResize);
        }
		...
		public function open():void
		{
			...
			opened = true;
			StageUtil.stage.focus = inputText;

			StageUtil.stage.addEventListener(KeyboardEvent.KEY_UP, keyUp);
			StageUtil.stage.addEventListener(KeyboardEvent.KEY_DOWN, keyDown);
		...
		public function close():void
		{
			...
			StageUtil.stage.removeEventListener(KeyboardEvent.KEY_UP, keyUp);
			StageUtil.stage.removeEventListener(KeyboardEvent.KEY_DOWN, keyDown);
			...
			if (str != "")
			{
				try
				{
					...
					if (areSuggestions)
					{
						...
						if (consoleHeight == StageUtil.stage.stageHeight)
			...
			else
			{
				...
				else
				{
					switch (str)
					{
						...
						case "help"		: echo(HELP, "#0ffd906"); 	break;
						case "list"		: echo(getCommandsList()); 	break;
//						case "author"	: echo(AUTHOR, "#0099CC");	break;
		...
		private function getCommandsList():String {
			var description:XML = describeType(main);
			...
			if (description.*.length() > 3) // if 3 (or less) that means ob doesn't exist
			{
				...
				for each (var a:XML in description.accessor)
				{
					...
					cmdSuggest.push(a.@name);
					result += a.@name + type + " <font color=\"#818181\">(accessor)</font><br>";
					areSuggestions = true;
				}
				// Methods / Functions
				for each (var m:XML in description.method)
		...
		{
			_parent.addEventListener(Event.ENTER_FRAME, slide);
		...
		private function stopSlideAnimation():void
		{
			...
			_parent.removeEventListener(Event.ENTER_FRAME, slide);
		...
		{
			if (consoleHeight != StageUtil.stage.stageHeight)
				consoleHeight = StageUtil.stage.stageHeight; // Extend
			else
			...
			if (slideAnimation)
			{
				_parent.addEventListener(Event.ENTER_FRAME, fullscreenSlide);
				slideAnimation_animating = true;
		...
		private function stopFullscreenSlideAnimation():void
		{
			...
			_parent.removeEventListener(Event.ENTER_FRAME, fullscreenSlide);
			slideAnimation_animating = false;
		}
